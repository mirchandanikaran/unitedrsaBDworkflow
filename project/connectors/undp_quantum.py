"""Connector for UNDP procurement notices portal."""
...
SOURCE = "undp_quantum"
BASE_URL = os.getenv("UNDP_QUANTUM_URL", "https://procurement-notices.undp.org/")
...
def _request_page() -> str:
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            last_error = exc
            logger.warning("UNDP request failed", extra={"source": SOURCE, "attempt": attempt, "error": str(exc)})
    if PLAYWRIGHT_FALLBACK:
        return fetch_rendered_html(BASE_URL)
    raise RuntimeError("UNDP request failed after retries") from last_error


def fetch_undp_quantum_tenders() -> list[dict[str, Any]]:
    """Return normalized tenders from UNDP procurement notices."""
    html = _request_page()
    links = extract_candidate_notice_links(
        html,
        base_url=BASE_URL,
        include_patterns=("procurement", "notice", "tender", "rfp", "itb", "eoi", "rfq"),
        max_links=MAX_LINKS,
    )
