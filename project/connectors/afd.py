"""Connector for Agence Francaise de Developpement tenders."""
...
def _request_page(page: int) -> str:
    """Fetch one AFD tenders listing page."""
    params = {"pageNo": page}
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            last_error = exc
            logger.warning("AFD request failed", extra={"source": SOURCE, "page": page, "attempt": attempt, "error": str(exc)})
    raise RuntimeError(f"AFD request failed for page={page}") from last_error


def fetch_afd_tenders(max_pages: int = MAX_PAGES) -> list[dict[str, Any]]:
    """Return normalized tenders from AFD listing."""
