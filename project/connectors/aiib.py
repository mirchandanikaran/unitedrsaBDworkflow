"""Connector for AIIB project procurement opportunities."""
...
MAX_LINKS = int(os.getenv("AIIB_MAX_LINKS", "1200"))
DETAIL_FETCH_LIMIT = int(os.getenv("AIIB_DETAIL_FETCH_LIMIT", "300"))


def _request_page() -> str:
    """Fetch AIIB procurement page HTML."""
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            last_error = exc
            logger.warning("AIIB request failed", extra={"source": SOURCE, "attempt": attempt, "error": str(exc)})
    if PLAYWRIGHT_FALLBACK:
        return fetch_rendered_html(BASE_URL)
    raise RuntimeError("AIIB request failed after retries") from last_error


def fetch_aiib_tenders() -> list[dict[str, Any]]:
    """Return normalized tenders from AIIB procurement listings."""
    html = _request_page()
    links = extract_candidate_notice_links(
        html,
        base_url=BASE_URL,
        include_patterns=("procurement", "project", "tender", "consulting", "notice"),
        max_links=MAX_LINKS,
    )
