"""Connector for IOM procurement opportunities."""
...
SOURCE = "iom"
BASE_URL = os.getenv("IOM_URL", "https://www.iom.int/procurement-opportunities")
...
MAX_LINKS = int(os.getenv("IOM_MAX_LINKS", "1200"))
DETAIL_FETCH_LIMIT = int(os.getenv("IOM_DETAIL_FETCH_LIMIT", "300"))


def _request_page() -> str:
    """Fetch IOM procurement page HTML."""
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            last_error = exc
            logger.warning("IOM request failed", extra={"source": SOURCE, "attempt": attempt, "error": str(exc)})
    if PLAYWRIGHT_FALLBACK:
        return fetch_rendered_html(BASE_URL)
    raise RuntimeError("IOM request failed after retries") from last_error


def fetch_iom_tenders() -> list[dict[str, Any]]:
    """Return normalized tenders from IOM procurement pages."""
    html = _request_page()
    links = extract_candidate_notice_links(
        html,
        base_url=BASE_URL,
        include_patterns=("procurement", "tender", "bid", "notice", "rfp", "eoi"),
        max_links=MAX_LINKS,
    )
